...
import random
import time
from common_items.use_variables import COMMON_LIST, not_global_number
# ---------------------------------------
def rand_pause():
...
def test_for_cuff_connection_negative(tenv: TEnv) -> None:
    ...
    with pytest.raises(ValueError) as exc_info:
        # noinspection PyCallingNonCallable
        tenv.pump.connected_cuff()
    assert str(exc_info.value) == "Inflatable is not connected"

def test_for_cuff_connection_negative_message(tenv: TEnv) -> None:
    tenv.pump._cuff = None
    rand_pause()
    with pytest.raises(ValueError) as exc_info:
        # noinspection PyCallingNonCallable
...
def test_for_cuff_connection_negative_message_text(tenv: TEnv) -> None:
    tenv.pump._cuff = None
    rand_pause()
    with pytest.raises(ValueError) as exc_info:
        # noinspection PyCallingNonCallable
